...
VIP_QR = "https://raw.githubusercontent.com/YOURUSER/YOURREPO/main/qr.jpg"
...
async def call(c):
    ...
    if uid in ADMINS and c.data=="admin_wd":
        await c.message.answer("Xem lịch sử rút trong DB")
...
async def steps(m):
    ...
    if USER_STEP.get(uid)=="wd":
        ...
        async with aiosqlite.connect("data.db") as db:
            ...
            await db.commit()
        for ad in ADMINS:
            await bot.send_message(ad,f"User {uid} rút {amount}")
        await m.answer("Đã gửi yêu cầu")
        USER_STEP.pop(uid)
